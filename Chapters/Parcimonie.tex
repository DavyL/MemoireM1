...
Précisons donc ce que nous avons affirmé, dans le même cadre que précedemment, c'est à dire dans le cas d'un dictionnaire $\Phi = (T,W)$ temps fréquence composé de la base de Fourier et de Diracdans $\mathbb{R}^N$.
...
\begin{preuve}
	...
	Supposons que $x$ vérifie les conditions du théorème, on doit donc montrer que si $x$ est l'unique solution de (P1), alors pour tout $x_1$ différent de $x$ qui vérifie $f = F x_1$ alors $||x_1||_1 > ||x||_1$.
	...
	\end{equation}
	On avait vu avec \ref{eq:structN} que les éléments $\delta$ de $\mathcal{N}$ sont de la forme $(\delta_T, -F_W^t F_T \delta_T) = (\delta_T, -\widehat{\delta_T})$, donc \ref{eq:ineqdelta} devient
	\begin{equation}
		\sum_{t\in \Gamma_T} |\delta_{T, t}| + \sum_{\omega \in \Gamma_W} |\widehat{\delta_T}_\omega|
		< \frac{||\delta_T||_1 + ||\widehat{\delta_T}||_1}{2}
	\end{equation}
	On peut ainsi définir la quantité
	\begin{equation}
		\mu(\Gamma_T, \Gamma_W) = \sup_x \frac{	\sum_{t\in \Gamma_T} |\delta_{T, t}| + \sum_{\omega \in \Gamma_W} |\widehat{\delta_T}_\omega|}{||\delta_T||_1 + ||\delta_W||_1}
	\end{equation}
	et ainsi la condition suffisante pour que la solution de \ref{P1} soit unique devient
	\begin{equation}
		\mu(\Gamma_T, \Gamma_W) < \frac{1}{2}.
	\end{equation}
	Avant de démontrer que cette inégalité est bien vérifiée énonçons ce que nous avons démontré qui est une généralisation du théorème \ref{th:DiracFourier} :
	\begin{theoreme}\label{th:muP1}
		Soit $N$ un entier positif et un signal $f \in \mathbb{R}^N$, alors n'importe quel $x = (x_T, x_W)$, où $\Gamma_T$ est le support de $x_T$ et $\Gamma_W$ est le support de $x_W$, 	vérifiant $f = F_T x_T + F_W x_W$ et
		\begin{equation}
			\mu(\Gamma_T, \Gamma_W) < \frac{1}{2}
		\end{equation}
	...
	\end{theoreme}
	\begin{preuve}
		Réécrivons une preuve correcte :
	\begin{equation}\label{eq:ineqmu}
	...
	\end{equation}
		Ce qui conclut la preuve (correcte?) du théorème.
